...
class TranzitMonth(BaseHoroscope):
    ...
    def calculate_aspect(degree1, degree2):
        ...
        if difference < 0.5:  # Погрешность +-0.5 градусов для учёта орбиса
            return 'соединение'
        elif abs(difference - 60) < 0.5:
            return 'секстиль'
        elif abs(difference - 90) < 0.5:
            return 'квадрат'
        elif abs(difference - 120) < 0.5:
            return 'тригон'
        elif abs(difference - 180) < 0.5:
            return 'оппозиция'
