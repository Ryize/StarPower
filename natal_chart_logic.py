...
class TranzitMonth(BaseHoroscope):
    ...
    description_con = (
        'На основе полученной информации составь астрологический прогноз на месяц. Опиши начало месяца, середину и конец. Как лучше провести каждое время чем лучше заниматься, чего опасаться, какие силы будут действовать, какие будут чувства и эмоции. При описании периодов не используй числа. Тебе нужно выстроить доверительную связь с клиентов во время сеанса'
    ...
    description = (
        ...
        ' какое время он будет действовать, как это повлияет на меня, что мне сделать что бы провести это время максимально продуктивно. Чего стоит опасаться или наоборот что сможет порадовать. '
...
class TranzitYear(BaseHoroscope):
    ...
    def __init__(self, birth_date, birth_place) -> None:
        ...
        self.birth_place = birth_place
        self.astralData = GetAstralData(self.birth_date, self.birth_place)
        self.len_year = lambda year: 366 if calendar.isleap(self.c_year) else 365
    ...
    def tranzit(self):
        ...
        for day in range(1, self.len_year(self.c_year) + 1):
            ...
            # позиция планет на данный день
            day_position_planet = dayAstralData.calc_planet_position(swe.URANUS)
            for natal_planet in self.personal_planets:
