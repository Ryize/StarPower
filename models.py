...
включая пользователей, гороскопы, натальные карты и базовые модели. Включает в себя методы
для управления данными, такие как создание, сохранение и извлечение записей
из базы данных, а также проверка входных данных новых пользователей.

Классы:
    BaseModel: Базовый класс модели, предоставляющий методы сохранения и создания экземпляров модели.
    User: Модель пользователя, содержащая информацию о пользователе, включая логин, электронную почту и пароль.
    UserNatalChart: Модель натальной карты пользователя.
    Horoscope: Модель гороскопа, содержащая информацию о прогнозах для различных периодов и знаков зодиака.
    DataAccess: Класс для управления доступом к данным, включающий методы для работы с пользовательскими данными,
                гороскопами и натальными картами.

Функции:
    load_user(user_id): Функция для загрузки пользователя по его идентификатору, используется flask_login для
                         управления пользовательскими сессиями.

Примеры использования:
    # Создание нового пользователя
    new_user = User.create(login='exampleUser', email='user@example.com', password='securePassword')

    # Получение гороскопа для определенного знака зодиака и периода
    horoscope = DataAccess().get_horoscope(period='today', date='2023-04-01', zodiac_sign='Aries')

Зависимости:
    Модуль зависит от Flask, Flask-Login, Flask-SQLAlchemy для работы с приложением, управления сессиями
    и взаимодействия с базой данных. Также используется Werkzeug для безопасной работы с паролями и регулярные выражения
...
class BaseModel:
    ...
    Атрибуты:
        id (int): Уникальный идентификатор для каждой записи, который автоматически
                  генерируется базой данных при создании новой записи.
        created_at (datetime): Дата и время создания записи, автоматически устанавливается
                               в момент создания новой записи.

    Методы:
        save(self) -> None:
            Сохраняет текущий экземпляр модели в базе данных. Если объект уже существует,
            он будет обновлен, в противном случае — создан новый.
    ...
    Пример использования:
        Наследуя этот класс, можно создать модель пользователя следующим образом:
    ...
    def save(self) -> None:
        ...
        Этот метод используется как для добавления новых объектов, так и для обновления существующих.
        """
        db.session.add(self)
        db.session.commit()
    ...
    def create(cls, *args, **kwargs) -> "BaseModel":
        """
        Создает новый экземпляр класса с заданными аргументами и сохраняет его в базе данных.
        ...
        Возвращает:
            Экземпляр класса, производного от BaseModel, представляющий новосозданную запись в базе данных.
        """
...
class User(db.Model, BaseModel, UserMixin):
    ...
    Атрибуты:
        ...
        birth_time (datetime.time): Время рождения пользователя. Необязательный.
        ...
        premium (bool): Статус премиум-подписки пользователя. По умолчанию False (не премиум).
        zodiac_sign (str): Знак зодиака пользователя, вычисляется на основе даты рождения. Необязательный.
        natal_chart (relationship): Связь с моделью UserNatalChart для доступа к натальной карте пользователя.
                                    Использует внешний ключ и ленивую загрузку.

    Использование:
        Для создания нового пользователя используйте метод create класса BaseModel.
    """
...
class UserNatalChart(db.Model, BaseModel):
    ...
    Атрибуты:
        user_id (int): Идентификатор пользователя, к которому относится натальная карта.
                       Устанавливается как внешний ключ на таблицу пользователей.
        natal_chart (str): Содержимое натальной карты в текстовом формате. Хранит информацию о планетарных
                           позициях, аспектах и других астрологических данных пользователя.

    Использование:
        Для добавления натальной карты пользователя, сначала необходимо создать или получить экземпляр пользователя,
        затем создать экземпляр UserNatalChart с ссылкой на этого пользователя и сохранить в базе данных.
    """
