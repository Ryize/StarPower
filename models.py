...
class Horoscope(db.Model, BaseModel):
    ...
    Args:
        period (str): Период прогноза (например, "сегодня", "неделя", "месяц").
        Может быть пустым, если прогноз не привязан к конкретному периоду
        времени.
        zodiac_sign (str): Знак зодиака, для которого предназначен гороскоп.
        Может быть пустым, еслигороскоп общий и не привязан к конкретному
        знаку.
        horoscope (str): Текст гороскопа. Содержит прогнозы и советы для
        указанного знака зодиака и периода. Должен быть заполнен.
        date (datetime.date): Дата, для которой актуален гороскоп. Помогает
        определить актуальность
                              прогноза.

    Использование:
        Для добавления нового гороскопа в базу данных, создайте экземпляр
        Horoscope с необходимыми атрибутами и используйте метод `create` для
...
class DataAccess:
    """
    Класс для управления доступом к данными в приложении прогнозирования
    гороскопов.
    Методы:
        check_new_user(self, login: str, email: str, password: str) -> bool:
            Проверяет данные нового пользователя на уникальность и
            соответствие форматам. Возвращает True, если проверка пройдена
            успешно и регистрация возможна.

        add_user(self, forms):
            Создает нового пользователя с данными из формы регистрации.

        get_user(self, login: str, password: str) -> bool | None:
            Пытается аутентифицировать пользователя с заданными логином и
            паролем.
    ...
    def check_new_user(self, login: str, email: str, password: str) -> bool:
        ...
        почты, а также соответствие логина, электронной почты и пароля
        заданным регулярным выражениям.

        Args:
            login (str): Логин нового пользователя. Должен быть уникальным и
                         соответствовать определенному формату (5-25 символов,
                         включая буквы, цифры и специальные символы).
            email (str): Электронная почта нового пользователя. Должна быть
                         уникальной и соответствовать стандартному формату
                         электронной почты.
            password (str): Пароль нового пользователя. Должен соответствовать
                            определенному формату (5-36 символов, включая
                            буквы, цифры и специальные символы).

        Returns:
            bool: True, если пользователь успешно прошел проверку и может быть
                  зарегистрирован. Возвращает False и выводит соответствующие
                  сообщения об ошибках через Flask flash, если какое-либо
                  условие не выполнено.

        Использование:
            result = check_new_user("newUserLogin", "user@example.com",
            "SecurePassword123!")
            if result:
        ...
        """
        # длина 5-25 символов.
        pattern_login = r'^.{5,25}$'
        # Регулярное выражение для email: стандартный формат email с доменом
        # верхнего уровня от 2 до 4 символов.
        pattern_email = r'^[-\w\.]+@([-\w]+\.)+[-\w]{2,4}$'
        # длина 5-36 символов.
        pattern_password = r'^.{5,36}$'
    ...
    def add_user(self, forms: dict) -> None:
        """
        Создает нового пользователя, хешируя пароль для безопасности перед
        сохранением.
    ...
    def get_user(self, login: str, password: str, remeber) -> bool | None:
        """
        Аутентифицирует пользователя на основе предоставленного логина и
        пароля.
        ...
        Returns:
            True: Если аутентификация прошла успешно (пользователь найден и
                  пароль верный).
    ...
    def add_profile(self, user: User, forms: dict) -> None:
        ...
        forms (dict): Словарь с данными формы. Ключи соответствуют полям
                      объекта пользователя.

        Действия:
            - Преобразует строку даты рождения в объект даты и обновляет поле
             `birthday`.
            - Преобразует строку времени рождения в объект времени и обновляет
              поле `birth_time`.
            - Вычисляет знак зодиака на основе новой даты рождения и обновляет
              поле `zodiac_sign`.
            - Обновляет остальные поля пользователя данными из формы, если они
              предоставлены.

        Returns:
            None. Метод обновляет данные в базе данных и не возвращает
            значение.
        """
    ...
    def add_avatar(self, current_user: User, file_path: str) -> None:
        """
        Устанавливает или обновляет путь к файлу аватара для текущего
        пользователя.

        Args:
        current_user (User): Объект пользователя, чей аватар необходимо
                             обновить.
        file_path (str): Строка, содержащая путь к новому файлу аватара
                         пользователя.

        Returns:
            None. Метод выполняет обновление данных в базе данных и не
            возвращает значение.
        """
        current_user.avatar = file_path
        db.session.commit()

    def get_horoscope(self, period: str,
                      date: datetime.date,
                      zodiac_sign: str) -> Horoscope:
        """
        ...
        Args:
            ...
            zodiac_sign (str): Знак зодиака, для которого запрашивается
                               гороскоп.

        Returns:
            Horoscope: Экземпляр модели `Horoscope`, соответствующий заданным
            критериям. Возвращает None, если подходящий гороскоп не найден.
        """
    ...
    def add_new_horoscope(self, period: str,
                          zodiac_sign: str,
                          text: str, date: datetime.date) -> None:
        """
        Добавляет новый гороскоп в базу данных.

        Args:
            period (str): Период гороскопа
                         (например, "день", "неделя", "месяц").
            zodiac_sign (str): Знак зодиака, для которого предназначен
                               гороскоп.
            text (str): Текст гороскопа, содержащий прогноз.
            date (datetime.date): Дата, на которую предоставлен прогноз.

        Returns:
            None. Метод не возвращает значение, но результатом его выполнения
            является добавление новой записи в таблицу гороскопов.
        """
    ...
    def get_natal_chart(self, user_id: int) -> UserNatalChart:
        ...
        Args:
            user_id (int): Идентификатор пользователя, натальную карту
                           которого необходимо получить.

        Returns:
            UserNatalChart: Экземпляр модели `UserNatalChart`, соответствующий
            заданному пользователю. Возвращает `None`, если натальная карта
    ...
    def add_new_natal_cart(self, user_id: int, text: str) -> None:
        ...
        Args:
            user_id (int): Идентификатор пользователя, для которого создаётся
            натальная карта.
            text (str): Текст натальной карты, содержащий астрологические
            данные и интерпретации.

        Returns:
            None. Метод не возвращает значение, но вносит изменения в базу
            данных, добавляя новую запись.
