...
Этот модуль содержит представления, которые обрабатывают запросы от пользователя и
возвращают соответствующие ответы. Он взаимодействует с моделями для получения данных и
передает эти данные в шаблоны для отображения пользователям. Каждое представление
ассоциировано с одним или несколькими URL-адресами.
...
def register() -> Response | str:
    ...
    Проверяет, что пароли совпадают и что пользователь с таким 'login' еще не
    зарегистрирован в базе данных. Если проверка прошла успешно, то создает нового
    пользователя, удаляя из данных формы 'confirm_password', и перенаправляет на страницу
    авторизации. В случае, если пароли не совпадают или пользователь с таким 'login' уже
    существует, пользователю выводится соответствующее сообщение об ошибке.

    Returns: render_template('register_authorization.html') в случае GET запроса или
             ошибки регистрации,
    ...
    if data_access.check_new_user(**forms):
        data_access.add_user(forms)
...
def authorization() -> Response | str:
    ...
    POST запрос:
    Извлекает из запроса 'login' и 'password'. Проверяет наличие пользователя с такими
    данными в базе данных. Если пользователь найден, происходит его авторизация. Для
    пользователя с ролью 'Admin' осуществляется перенаправление на административную
    панель. В случае успешной авторизации выводится приветственное сообщение и
    перенаправление на главную страницу. Если пользователь не найден, выводится сообщение
    об ошибке.
    ...
    if data_access.get_user(login, password, remember):
...
def profile() -> Response | str:
    ...
    POST запрос:
    Принимает измененные данные профиля пользователя из формы. Если данные о времени
    рождения или дате рождения изменены, существующая натальная карта пользователя
    удаляется из базы данных, предполагая необходимость создания новой. После обновления
    данных профиля в базе данных, пользователю выводится сообщение об успешном сохранении
    изменений и происходит перенаправление на страницу профиля.

    Returns: render_template('profile.html') в случае GET запроса, с данными о времени
             рождения, если оно указано,
             redirect(url_for('profile')) после обновления данных профиля в случае POST
             запроса.
    """
    if request.method == "GET":
    ...
    user = current_user
    if (user.birth_time != request.form["birth_time"] or
            user.birthday != request.form["birthday"]):
        # Удаление не актуальной натальной карты
        data_access.del_natal_chart(user.id)
    # Добавление данных в профиль текущего пользователя
    data_access.add_profile(user, request.form)
    flash(
...
def upload():
    ...
    и соответствует разрешенным типам файлов. Если файл валиден, предыдущий аватар
    пользователя удаляется (если он существует), и загруженный файл сохраняется в заданной
    директории с уникальным именем файла, основанным на текущем времени. Путь к новому
    файлу аватара добавляется в профиль пользователя в базе данных. Пользователь получает
    уведомление об успешной загрузке файла.

    В случае ошибки (если файл не выбран, не соответствует разрешенным типам или другая
    ошибка загрузки), пользователю отображается соответствующее сообщение об ошибке.

    Returns: redirect(url_for('profile')) возвращает пользователя на страницу профиля
    ...
    data_access.add_avatar(current_user, file_path)
    flash(
...
def horoscope(period) -> Response | str:
    ...
    Принимает параметр 'period', который определяет период времени для гороскопа
    (например, "день", "неделя"). Перед выводом гороскопа проверяет, заполнены ли данные о
    дате рождения и времени рождения пользователя. Если данные не заполнены, отображается
    сообщение с просьбой их заполнить и происходит перенаправление на страницу профиля для
    дополнения информации.

    Если данные пользователя заполнены, осуществляется поиск гороскопа для данного периода
    и знака зодиака пользователя. В случае отсутствия гороскопа в базе данных,
    производится запрос к внешнему сервису для получения гороскопа, который затем
    сохраняется в базе данных для последующего использования.

    Param: period: Строка, указывающая период гороскопа (например, "today", "week").
    Returns: render_template('horoscope_chat.html') с гороскопом для заданного периода,
    ...
    horoscope = data_access.get_horoscope(period, date, zodiac_sign)
    if horoscope:
        return jsonify(
            {
                "success": True,
    ...
    data_access.add_new_horoscope(period, zodiac_sign, text, date)
    return jsonify(
        {
            "success": True,
            "text": text,
...
def special_horoscope() -> Response | str:
    ...
    Из формы запроса извлекается специальная дата ('sp_date'), для которой должен быть
    создан гороскоп. Проверяет наличие гороскопа на эту дату в базе данных. Если гороскоп
    не найден, осуществляется запрос к внешнему сервису для получения гороскопа, который
    после получения сохраняется в базу данных.

    Returns: render_template('horoscope_chat.html') с текстом гороскопа,
             или перенаправление на страницу профиля, если требуется заполнение данных
             пользователя.
    """
    # сделать проверку данных и перекинуть для заполнения на profile
    if request.method == 'GET':
    ...
    zodiac_sign = current_user.zodiac_sign
    # Поиск подходящего гороскопа по заданным параметрам в БД
    horoscope = data_access.get_horoscope(period, sp_date, zodiac_sign)
    if not horoscope:
        get_horoscope = GetSpecialHoroscope(sp_date, zodiac_sign)
        text = get_horoscope.get_response()
        # Добавление нового гороскопа в БД
        data_access.add_new_horoscope(period, zodiac_sign, text, sp_date)
        return jsonify(
            {
                "success": True,
                "text": text,
...
def natal_chart() -> Response | str:
    ...
    GET запрос:
    Перед отображением натальной карты проверяет, заполнены ли необходимые данные
    пользователя (дата рождения и время рождения). Если данные не заполнены, выводит
    сообщение с просьбой заполнить данные и перенаправляет на страницу профиля. В
    противном случае отображает страницу для генерации натальной карты.

    POST запрос:
    Генерирует натальную карту на основе данных пользователя, включая дату и время
    рождения, а также место рождения. Если натальная карта для пользователя уже существует
    в базе данных, возвращает существующую карту. В противном случае происходит создание
    новой натальной карты, которая затем сохраняется в базу данных.

    Returns: при GET запросе возвращает страницу для генерации натальной карты,
    ...
    natal_cart = data_access.get_natal_chart(current_user.id)
    if natal_cart:
        return jsonify(
            {
                "success": True,
    ...
    data_access.add_new_natal_cart(current_user.id, text)
    return jsonify(
        {
            "success": True,
            "natal_chart": text,
...
def redirect_to_sign(response):
    ...
    Эта функция вызывается автоматически после каждого запроса. Она проверяет статусный
    код ответа. Если статусный код ответа равен 401 (Неавторизован), происходит
    перенаправление пользователя на страницу регистрации и авторизации. Это обеспечивает,
    что пользователи, пытающиеся получить доступ к защищенным ресурсам без соответствующих
    прав доступа, будут направлены к форме входа, вместо отображения стандартной страницы
    с ошибкой 401.

    Param: response: Объект ответа Flask, который был сгенерирован обработчиками запросов.
    Returns: Исходный объект ответа или объект перенаправления на страницу авторизации.
    """
